...
module.exports = function(RED) {
	...
	function OrionService(n){
        ...
        this.queryContext = function(node, n, payload){
    		...
    		return when.promise(function(resolve,reject) {
    			var url = orionUrl + "/v1/queryContext?limit=" + n.limit + "&details=on";
...
//    			console.log("payload: " + payload);
        ...
        this.createContext = function (node, payload){
        	node.status({fill:"blue",shape:"dot",text:"sending request"});
        	console.log("----------createContext payload: " + JSON.stringify(payload));
        	return when.promise(function(resolve,reject) {
	        	request(node, "POST", payload, orionUrl + "/v1/updateContext").then(function(result){
        ...
        this.subscribe = function (node, payload) {
    		...
    		var url = orionUrl + "/v1/subscribeContext";
    		payload = JSON.stringify(payload);
    		...
    		var req = ((/^https/.test(url))?https:http).request(opts,function(res) {
    			...
    			res.on('end',function() {
   			        var subscribeResponse = JSON.parse(msg.payload).subscribeResponse;
			...
			listenOnUrl(nodeID, function(req,res) {
				...
				if(req.body.subscriptionId != subscriptionID){
	    			...
	    			node.send({payload: payload, statusCode: 200});
				}
    		});

    		node.status({fill:"blue",shape:"dot",text:"listening on " + reference});
	...
	function request(node, method, payload, url) {
		...
		return when.promise(function(resolve,reject) {
			...
			req.on('error',function(err) {
				reject(err);
				node.status({fill:"red",shape:"ring",text:err.code});
				node.send({payload: err.toString() + " : " + url, statusCode: err.code});
			});
		...
		return when.promise(function(resolve,reject) {
			listenOnUrl(nodeID, function(req,res){
                ...
                if(result.subscriptionId != testSubscriptionID){
                	node.error("Communication with context broker failed");
                	...
                	console.log("result.subscriptionId: " + result.subscriptionId + "==" + testSubscriptionID);
                	resolve();
                }
            });
			...
			request(node, "POST", createElementPayload, orionUrl + "/v1/updateContext").then(function(result){
				...
				getSubscribeTestPayload(node).then(function(subscribePayload){
					request(node, "POST", subscribePayload, orionUrl + "/v1/subscribeContext").then(function(subscription){
	                    ...
	                    try {
	    					testSubscriptionID = subscription.subscribeResponse.subscriptionId;
	...
	function OrionSubscribe(n) {
		...
		node.brokerConn.init(node, n).then(function(){
			getSubscribePayload(node, n).then(function(payload){
				node.brokerConn.subscribe(node, payload);		
			});
		});
	}
    ...
    function getMyUri(node){
    	return when.promise(
			function(resolve,reject) {
		    	...
		    	if(myUri){
		    		resolve(myUri + ":" + RED.settings.uiPort);
		    	}else{
				...
				node.brokerConn.init(node, n).then(function(){
					node.brokerConn.queryContext(node, n, payload).then(
							function(msg){
								...
								node.send({payload: msg, statusCode: 200});
							},
							function(reason){
    ...
    function OrionTest(n) {
		...
		this.on("input",function(msg) {
			...
			try {
				node.brokerConn.init(node, n).then(function(){
					node.brokerConn.createContext(node, payload).then(
							function(msg){},
							function(reason){
